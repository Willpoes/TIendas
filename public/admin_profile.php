				...
				<div class="display-table-cell">
					<div class="login-container">
						...
						<form action="../private/update_admin.php" method="POST">
							...
							<?php
                            if ($admin['last_name'] == NULL && $admin['mother_last_name'] == NULL) {
                                
                                ?>
                                <input class="" type="hidden" name="last_name" placeholder="Apellidos" value="" readonly>
                                <?php
                            } else if($admin['last_name'] == " "){
                            ?> 
                            <input class="" type="hidden" name="last_name" placeholder="Apellidos" value="<?php echo trim($admin['last_name']) ?>" readonly>
                            <?php
                            }else {
                                ?>
                                <input class="" type="hidden" name="last_name" placeholder="Apellidos" value="<?php echo $admin['last_name'] . " " . $admin['mother_last_name'] ?>" readonly>
                            <?php } ?>
							...
							<h3 style="color: red; font-size: 1.3em; margin-top: 1em; font-family: 'verdana';">Complete los siguiente campos para poder registar Productos!</h3>
...
<h6 class="validation-msg"></h6>
<input type="hidden" class="" type="text" name="username" placeholder="Usuario" value="<?php echo $admin['username']?>">

<!-- ===== Add Fields ====  -->
<input name="fgallery" id="fgallery" placeholder="GALERIA*" class="form-control" type="text" required >
<input name="fdirection" id="fdirection" placeholder="Direccion de la Galeria *" class="form-control" type="text" required > 
...
<!--add fields datos personales -->

                            <input type="hidden" name="first_name" placeholder="Nombres" value="<?php echo $admin['first_name']?>" readonly >
							<?php
                            if ($admin['last_name'] == "" && $admin['mother_last_name'] == "") {
                                ?>
                                <input type="hidden" name="last_name" placeholder="Apellidos">
                                <?php
                            } else {
                                ?>
                                <input type="hidden" type="text" name="last_name" placeholder="Apellidos" value="<?php echo $admin['last_name'] . " " . $admin['mother_last_name'] ?>">
                            <?php } ?>
				            
                            <input class="" type="hidden" name="number" placeholder="Télefono" value="<?php echo $admin['mobile']?>" readonly>
...
<button class="submit btn" type="submit"><b>ACTUALIZAR</b></button>
						...
						//header("Location: https://fulventas.com/public/recent_products.php", TRUE, 301);
                        //exit();
