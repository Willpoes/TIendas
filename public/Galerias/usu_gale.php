...
$business_name = '';
$address = '';
$type = 4; // Asignar un tipo fijo de 4
$ruc = '';
$bank_account = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($resultado !== false) {
        ...
        if (isset($_GET['edit'])) {
            ...
            $guardar = $con->prepare("UPDATE users SET first_name = ?, last_name = ?, mother_last_name = ?, username = ?, email = ?, password = ?, mobile = ?, department = ?, province = ?, district = ?, business_name = ?, address = ?, ruc = ?, date = ?, bank_account = ? WHERE user_id = ?");
            $guardar->execute([$first_name, $last_name, $mother_last_name, $username, $email, $password, $mobile, $department, $province, $district, $business_name, $address, $ruc, $fecha, $bank_account, $user_id]);
    } else {
            // Insertar nuevo usuario
            $guardar = $con->prepare("INSERT INTO users (first_name, last_name, mother_last_name, username, email, password, mobile, department, province, district, business_name, address,type, ruc, bank_account, status,date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?,NOW())");
            $guardar->execute([$first_name, $last_name, $mother_last_name, $username, $email, $password, $mobile, $department, $province, $district, $business_name, $address,$type, $ruc, $bank_account, $status]);
        }
